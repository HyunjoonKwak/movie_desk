...
| 파형이 클립 중앙에 겹침 | 그림 있는 클립은 아래 40% 띠에 파형 | `eabaa53` |
| 내보내기 보고서의 색 근사·리미터 안내 | 내보내기는 정확 프레임을 기다림(`exact`), 색은 컨테이너에서 읽어 검증, 리미터 문구 수정 | `6a447bc` |
| 한 보관소에서 여러 컷 | 컷 = 최상위 타임라인, 전환은 루트 교체 (`docs/decisions/2026-09-11-multiple-cuts.md`) | `5738b18` |
