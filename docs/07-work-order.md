...
| A5 후속 미디어 패널 가상화 | Codex 구현 | 구현·자동 검증 완료 | 날짜 그룹·단일 목록을 IntersectionObserver 행 세그먼트로 가상화하고 마퀴를 순수 레이아웃 모델로 계산. 페인트·스킵 카드 혼합 스크롤 높이 드리프트 0px, 좁은 폭 헤더 21px 유지. 1,000개 필터 345→48ms, 검색 37ms, DOM 카드 16장, 힙 116/51MB, 가져오기 7.4ms/자산. 새로고침 준비 510ms로 <400ms 목표는 미달해 복원 경로 후속 측정 필요 |
