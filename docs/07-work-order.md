...
- 미디어 카드의 "썸네일 다시 만들기": `codex/a5-reload-path` 구현. 현재 원본으로 썸네일·필름스트립·파형을
  재생성하고 프리뷰 저장소와 카드에 반영한다. 누락 소스에서는 숨기고 진행·성공·실패 토스트를 제공한다.
- 스냅샷 보존 상한: `codex/a5-reload-path` 구현. 프로젝트별 최근 20개를 남기는 오래된 후보 수를 표시하고
  확인 후 해당 ID만 정리한다. 저장 시 자동 삭제하지 않으며 GC는 삭제 후 남은 스냅샷만 보존한다.
...
| A5 새로고침·프리뷰 백로그 | Codex | 구현·측정, 400ms 목표 미달·리뷰 대기 | `codex/a5-reload-path`: Yjs 중복 반영 차단·소스 첫 검사 지연, 프리뷰 재생성, 최근 20개 확인식 스냅샷 정리. dev 675→616ms·prod 528ms, gate PASS·E2E 45개. 측정·목표 판정: [평가 문서](evaluations/2026-09-06-reload-path.md) |
