...
| 영역 | 확인된 현재 상태 |
| --- | --- |
| 제품 | Movie Desk 정체성·이름·저장소·랜딩 정리 완료. 제품 플랫폼은 Photo Desk와 같은 macOS 로컬 앱으로 확정했다. D1(파일 참조 + SQLite 카탈로그)·D2(데스크톱 매니페스트 canonical)·D3(데스크톱 우선, 스파이크 후)·D4(이름) 결정 완료. |
| 브랜치 | Phase 0~5의 배치가 모두 `main`(8209171)에 통합됐다. 열린 작업 브랜치는 없다. Codex는 사용량 제한으로 잠시 빠져 있고 그동안 Claude가 전 영역을 맡는다. |
| CI | `main`의 CI(lint·typecheck·단위·OSV·프로덕션 빌드·Playwright)가 통과한다. 로컬은 `pnpm gate` 한 번으로 같은 순서를 돌린다(`docs/09-release-checklist.md`). |
| 검증 | 단위 web 385 + core 106 + desktop 56 + scripts 11, Playwright 21개(내보내기·취소·스냅샷·충돌 복구·손상 프로젝트·저장 공간 부족·누락 미디어·자동 편집·WebCodecs 샘플러) + 로컬 Chrome HEVC 1개. |
| 릴리스 | 버전은 `0.4.0-rc.1`로 동기화됐고 태그만 남았다. 옛 `cut_editor` 데이터 채택과 rc를 인식하는 업데이트 확인이 들어 있다. 여전히 미서명 DMG다. |
| 입력 | MP4·MOV(HEVC 포함, 회전)·HEIC(데스크톱)·Live Photo·폴더 가져오기, 실패 분류·재시도, 오디오 variant, mediabunny 단일 데먹서. 실제 iPhone HDR/VFR·대량 폴더는 도그푸딩 검증 대기. |
| 핵심 품질 | 자동 정리·리포트·컷 이유·5개 모드·고정 시나리오·카드 템플릿·한국어 Whisper·공유 프리셋·완료 화면이 구현돼 있지만 실제 촬영본 도그푸딩 기록은 아직 없다. |
| 신뢰성 | 누락 미디어는 카드 배지·프리뷰 안내·내보내기 거부로 드러난다. 디코더 핸들 LRU, 재시도 backoff, 내보내기 사전 점검, 릴리스 체크리스트 자동화. 재연결 UI(A4)는 아직 없다. |
| 기술 부채 | knip 0건. `mp4-muxer`·`mp4box`는 mediabunny로 대체됐다. 선택형 MobileCLIP 의미 태깅은 설정 UI가 없어 꺼져 있다(켤지 결정 필요). |
| 오프라인 | 미디어는 외부로 전송하지 않는다. 데스크톱 빌드는 FaceLandmarker·Whisper(다국어 base)·ONNX 런타임을 사전 포함하고 `app://` 실행의 조용한 원격 폴백을 차단한다. |
...
1. `v0.4.0-rc.1` 태그를 `main` 8209171에 푸시하고 초안 Release의 arm64·x64 DMG를 확인.
2. B7 도그푸딩 1회차: `docs/dogfood/TEMPLATE.md` 절차와 `docs/09-release-checklist.md` 3절 수동 항목 완주.
3. 도그푸딩 P0를 B8로 배정해 다른 계획보다 먼저 수정.
4. 실기기 검증 닫기: iPhone HEIC·Live Photo·HDR/VFR MOV·대량 폴더, 실제 음성 자막, 메신저·TV 재생, `cut_editor` 데이터 채택.
5. 두 번째 촬영본 세트 완주 후 `v0.4.0` 태그와 업데이트 안내 확인(B25).
6. A4 누락 미디어 재연결·휴지통: 누락 배지에서 바로 다시 연결(지문 확인)하고 삭제한 자산을 되돌릴 수 있게.
7. A2 메타데이터 인덱스·복합 검색(촬영 시각·장소·코덱·길이·태그).
8. 프리뷰의 누락 자산 재시도 상한과 프록시·캐시·워커 측정(B'6)으로 대량 라이브러리 성능 기준선 확보.
9. C1 새 프로젝트 출발점 화면과 C2 빈·선택·오류 상태 설명.
10. 공개 배포 전 코드 서명·공증과 상표 확인(개인 사용 완주 품질이 먼저).
