...
29. **보존 클립 배치·폐기 UI** — Phase 1+7 5라운드(`3ef435b`)에서 넘어온 항목.
    병합으로 트랙 위치를 잃은 클립은 프로젝트와 JSON 백업에 `preservedClips`로
    보존되지만, 타임라인에도 완성 영상에도 나타나지 않고 사용자가 되살리거나
    버릴 방법이 없다. 그래서 개수 알림이 **세션마다 다시 뜬다.**
    되살리기(트랙 선택 후 배치)와 폐기(확인 후 영구 삭제) 두 경로가 필요하며,
    폐기는 되돌릴 수 없으므로 확인을 받아야 한다.
