...
- [ ] **누락 미디어:** 원본을 담은 외장 드라이브를 분리 → 내보내기가 파일 이름을 들어 거부한다
      → 다시 연결 → 내보내기가 성공한다. 미디어 카드의 누락 배지와 복구 안내도 확인한다 (5절).
...
### A4 데스크톱 참조 재연결·카탈로그 복원

- [ ] 일괄 미리보기에서 동일 지문만 기본 선택되고 다른 지문·크기는 미선택이다. 선택하지 않은 행은 연결되지 않는다.
- [ ] 대량 폴더 준비에서 진행률을 확인하고 취소한다. 1,000개 초과와 2분 제한은 복구 안내를 표시한다.
- [ ] 숨긴 창에서는 자동 디스크 검사가 멈추고, 표시된 창에서는 30초 간격으로 디스크 자산만 검사한다.
- [ ] 변경 없는 카탈로그는 반복 백업을 생략하고 세대별 최대 7개, 복원 전 사본 최대 2개를 유지한다.
- [ ] 백업 작업이 멈춘 상태에서도 종료 요청 뒤 약 4초 이내에 앱이 종료된다.


- [ ] RC DMG에서 전용 테스트 외장 USB의 원본을 참조로 가져온 뒤 물리 분리 → 재연결: 누락 안내와 카드 배지가 자동 해제되고 재생/내보내기가 복구된다. 사용자 작업 중인 드라이브는 분리하지 않는다.
- [ ] 누락 카드 → 다시 연결 → 네이티브 파일 선택: 같은 지문은 즉시 연결, 같은 크기의 다른 지문은 확인 전 연결되지 않는다. 취소 시 기존 참조가 유지된다.
- [ ] 같은 상대 경로 구조의 폴더를 선택: 여러 후보·불일치·사용 불가 행을 미리 확인하고 적용, 일부 실패는 다시 선택해 복구한다. 하위 폴더 구조가 유지된다.
- [ ] 원본·대체 후보의 파일 크기/해시가 재연결 전후 동일하고, 카드 썸네일·프록시·waveform에 이전 원본의 잔상이 없다.
- [ ] File → Back up media catalog 및 정상 종료 스냅샷 확인. 격리된 테스트 프로필의 DB를 손상시켜 복원 제안 표시, 취소 시 유지, 스냅샷 선택·확인 후 재시작과 카탈로그 복구를 확인한다.
- [ ] 복원 전 DB/WAL/SHM 보존 폴더가 남고 원본 미디어·프로젝트 파일이 바뀌지 않는다. 손상된 스냅샷은 거부되며 이전 카탈로그가 보존된다.
...
## 5. 알려진 빈틈 (2026-09-07 실제 코드 재확인)

이 절은 릴리스 전마다 실제 코드로 재확인한다.

2026-09-03에 적었던 네 항목의 현재 상태는 아래와 같다. 당시 남아 있던
버전 오류 예외의 영어 진단 문자열도 이번 릴리스 정리에서 기계 판독 코드로
바꿨다. 이 네 항목 중 현재 미해결 빈틈은 없다. RC DMG 수동 검증은 3절을 따른다.

| 기존 항목 | 해결일·근거 커밋 | 현재 코드 근거 (`apps/web/src/` 기준) |
| --- | --- | --- |
| 파일이 사라져도 카드가 정상처럼 보임 | 2026-09-04 `173a4db`; 카드 분리 `f355823` | `media/components/missing-badge.tsx:19`가 정상 외 상태를 표시하고 `media/components/media-card.tsx:191`이 `health`로 렌더한다 |
| 컴포지터 1초 간격 재시도 | 2026-09-03 `f1cb0f7`, 토큰 변경 즉시 재시도 `19d79c3` | `renderer/retry-backoff.ts:15`의 1초→30초 상한, `:22`의 토큰 변경 초기화, `:31`의 지수 증가. 재시도 자체는 복구를 위해 계속한다 |
| 스냅샷 단위 테스트 없음 | 2026-09-04 `749a92e` | `persistence/__tests__/snapshots.test.ts:85` 저장·목록·복원, `:99` 손상·없는 행, `:119` 삭제 검증 |
| 버전 불일치 영어 하드코딩 | 사용자 문구는 2026-09-04 `749a92e`에서 이미 해결; 예외 진단 정리는 2026-09-07 `2d449ac` | `editor/project-menu.tsx:155`가 방향별 i18n 키를 사용한다. `i18n/messages.en.ts:716`·`messages.ko.ts:714`에 두 방향 번역이 이미 있다. `persistence/project-export.ts:294` 예외에는 direction·fileVersion·appVersion과 진단 코드만 남기고 `e2e/project-version.spec.ts`가 en/ko 두 방향과 열린 프로젝트 보존을 검증한다 |

### B7 첫 완성률 도그푸딩

- [ ] 프로젝트 메뉴에서 측정 토글 켜고 새 프로젝트 완주 후 리포트 확인(C3, 로컬 전용).
