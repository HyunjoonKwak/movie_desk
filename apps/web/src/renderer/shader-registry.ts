import { type GL, linkProgram } from "./gl";
import { managedShader } from "./managed-shader";
import { fs as transferFs } from "./shaders/transfer";
...
export class ShaderRegistry {
  ...
  get(name: string, managed = false): Program {
    const key = managed ? `${name}:managed` : name;
    const cached = this.cache.get(key);
    if (cached) return cached;
    const fs = SHADERS[name];
    if (!fs) throw new Error(`Unknown shader: ${name}`);
    const handle = linkProgram(this.gl, VS_QUAD, managed ? managedShader(name, fs) : fs);
