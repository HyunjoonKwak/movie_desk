...
test("context restoration recreates color targets and reports approximation state", async ({
  page,
}) => {
  ...
  await page.keyboard.press("e");
  await expect(page.locator("[data-preview-canvas]")).toHaveAttribute("data-color-code", "255");
  await page.evaluate(() =>
  ...
  await expect(page.getByTestId("color-processing-hint")).toContainText("fallback.mov");
  await expect(page.getByTestId("color-processing-hint")).toContainText(
    "browser’s SDR approximation",
  );
  await page.locator("[data-preview-canvas]").evaluate((element) => {
    ...
    (element as HTMLCanvasElement).dataset.colorCode = "lost";
  ...
  await promisify(execFile)(
    process.execPath,
    [fileURLToPath(new URL("../../../scripts/color/managed.mjs", import.meta.url)), "--verify"],
    { timeout: 90000 },
  );
});
